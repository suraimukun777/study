#!/usr/bin/env python
"""
POT-SAM2 Hybrid Validation Script (Simple Version)
SAM2を使わず、POT-CAMから直接argmaxでマスクを生成
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='POT-SAM2 Validation (Simple)')
    ...
    parser.add_argument('--output_dir', type=str,
                       default='/workspace/POT_SAM2_Hybrid/experiments/validation_simple',
...
def run_validation(model, dataloader, args):
    """Validation実行（簡易版: POT-CAMのargmaxのみ）"""
    ...
    print("\n" + "="*60)
    print("Running validation (Simple Version - POT-CAM argmax only)...")
    print("="*60 + "\n")
    ...
    with torch.no_grad():
        for idx, pack in enumerate(tqdm(dataloader, desc="Validation")):
            ...
            # 推論（return_loss=Trueでトレーニングモード、CAMを取得）
            try:
                ...
                # argmaxでマスクを生成（簡易版）
                pred_mask = cam[0].argmax(dim=0).cpu().numpy().astype(np.uint8)
    ...
    # 結果を集計
    print("\n" + "="*60)
    print("Validation Results (Simple Version)")
    print("="*60 + "\n")
    ...
    if len(valid_class_ious) > 0:
        ...
        with open(result_path, 'w') as f:
            f.write("POT-SAM2 Hybrid Validation Results (Simple Version)\n")
            f.write("="*60 + "\n\n")
...
def main():
    ...
    print("\n" + "╔" + "="*58 + "╗")
    print("║" + " "*58 + "║")
    print("║" + "  POT-SAM2 Hybrid Validation (Simple Version)".center(58) + "║")
    print("║" + "  (POT-CAM argmax only, NO SAM2)".center(58) + "║")
    print("║" + " "*58 + "║")
    print("╚" + "="*58 + "╝\n")
